...
# 通过 streamlit.secrets 模块获取 Github 仓库内的文件
repo_content = st.secrets["github"]["Zoumachuan/CHN_EPW/CHN_EPW"]
folder_list = [item["name"] for item in repo_content if item["type"] == "dir"]
...
# 创建一个下拉框选择文件夹
selected_folder_index = st.selectbox("Select a province/选择省份", range(len(folder_list_replaced)),
                                     format_func=lambda i: folder_list_replaced[i])
...
# 获取文件夹内的所有文件
file_list = [item["name"] for item in repo_content if item["type"] == "file"]
...
if selected_file.endswith(".zip"):
    # 通过 streamlit.secrets 模块获取zip文件
    zip_data = st.secrets["github"]["Zoumachuan/CHN_EPW/CHN_EPW/" + original_selected_folder + "/" + selected_file]
